...
def get_published_article(db: Session, article_id: int):
    """Get a single published article by ID.
    
    Returns None if article doesn't exist or is not published.
    """
    return (
        db.query(Article)
        .filter(
...
def update_article(db: Session, article_id: int, **kwargs):
    ...
    if article:
        try:
            # Handle status change to update published_at
            if 'status' in kwargs:
                new_status = kwargs['status']
                old_status = article.status
                
                # If publishing (draft -> published), set published_at
                if old_status != "published" and new_status == "published":
                    kwargs['published_at'] = datetime.utcnow()
                # If unpublishing (published -> draft), clear published_at
                elif old_status == "published" and new_status != "published":
                    kwargs['published_at'] = None
                # If re-publishing (published -> published), update published_at (latest)
                elif old_status == "published" and new_status == "published":
                    kwargs['published_at'] = datetime.utcnow()
