...
// Watch for form.body changes to update editor
watch(() => form.value.body, (newBody) => {
  if (editor.value && newBody !== editor.value.getHTML()) {
...
async function handleImageUpload(event) {
  ...
  try {
    ...
    // Upload to server using imageService
...
async function saveContent() {
  ...
  const body = editor.value ? editor.value.getHTML() : form.value.body
  if (!body || body === '<p></p>') {
